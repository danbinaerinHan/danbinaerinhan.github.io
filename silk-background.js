...
function createDandelionDecor() {
  ...
  style.textContent = [
    ...
    '#dandelion-wrap{position:fixed;bottom:-7vh;left:2vw;height:65vh;z-index:1;cursor:default;pointer-events:none;transition:transform .5s ease}',
    '#dandelion-wrap:hover{transform:none}',
    ...
    // Hover label (hint disabled — pending permission)
    ...
    '@media(max-width:768px){#dandelion-wrap{height:45vh;left:auto;right:-5vw;bottom:-5vh}#dandelion-label{font-size:0.65rem}#dandelion-hint{font-size:0.9rem;bottom:5%}}'
  ].join('');
